...
if [ ! -f "/home/ark/.config/.update11232025" ]; then

  printf "\nAdd script to prettify /roms/ports/gamelist.xml, add text_viewer to /opt/inttools \n" | tee -a "$LOG_FILE"
  sudo wget --no-check-certificate https://github.com/wummle/arkos/raw/main/11232025/arkosupdate11232025.zip -O /home/ark/arkosupdate11232025.zip -a "$LOG_FILE" || rm -f /home/ark/arkosupdate11232025.zip | tee -a "$LOG_FILE"
  if [ -f "/home/ark/arkosupdate11232025.zip" ]; then
    sudo unzip -X -o /home/ark/arkosupdate11232025.zip -d / | tee -a "$LOG_FILE"
    sudo rm -v /home/ark/arkosupdate11232025.zip | tee -a "$LOG_FILE"
  else
    printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
    sleep 3
    echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
